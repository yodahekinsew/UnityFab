...
namespace TMPro
{
    ...
    /// </summary>
    public class CustomInputField : Selectable,
        ...
        IScrollHandler
    {
        public UndoRecorder undoRecorder;
        ...
        // {
        //     WebGLClipboard.ReadFromClipboard("clipboard", "HandlePasteFromWebGLClipboard");
        // }

        // public void HandlePasteFromWebGLClipboard(string pasteValue)
        // {
        //     clipboard = pasteValue;
        // }

        // public void CopyToClipboard(string copyValue)
        // {
        //     clipboard = copyValue;
        //     WebGLClipboard.WriteToClipboard(copyValue);
        // }
        ...
        {
            base.Start();
            undoRecorder.Record(new Record(m_Text, m_StringPosition, m_StringSelectPosition));
        }
        ...
        protected virtual void LateUpdate()
        {
            if (Input.GetKeyDown(KeyCode.Tab) &&
                !(Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift))) // Manually forcing a tab NOT GOOD LOL
            {
        ...
        protected EditState KeyPressed(Event evt)
        {
            ...
            bool ctrl = (currentEventModifiers & EventModifiers.Command) != 0 || (currentEventModifiers & EventModifiers.Control) != 0;
                ...
                case KeyCode.Z:
                    {
                        ...
                        {
                            Record record;
                            if (shift) record = undoRecorder.Redo();
                            else record = undoRecorder.Undo();
                            m_Text = record.text;
                            m_StringPosition = record.stringPosition;
                            m_StringSelectPosition = record.stringSelectPosition;
                            return EditState.Continue;
                        }
                        break;
                    }
            ...
            if (c == '\u0019') return EditState.Continue;
        ...
        {
            int stringPositionLine = m_TextComponent.textInfo.characterInfo[m_StringPosition].lineNumber;
            int stringSelectPositionLine = m_TextComponent.textInfo.characterInfo[m_StringSelectPosition].lineNumber;
            if (stringPositionLine == stringSelectPositionLine) Append("  ");
            else
            {
                int minLine = (int)Mathf.Min(stringPositionLine, stringSelectPositionLine);
                int maxLine = (int)Mathf.Max(stringPositionLine, stringSelectPositionLine);
                int tempStringPosition = Mathf.Min(m_StringPosition, m_StringSelectPosition);
                int tempStringSelectPosition = Mathf.Max(m_StringPosition, m_StringSelectPosition);
                int tempCaretPosition = Mathf.Min(m_CaretPosition, m_CaretSelectPosition);
                int tempCaretSelectPosition = Mathf.Max(m_CaretPosition, m_CaretSelectPosition);
                int numLine = 0;
                for (int i = minLine; i <= maxLine; i++)
                {
                    int lineStartingChar = m_TextComponent.textInfo.lineInfo[i].firstCharacterIndex + 2 * numLine;
                    m_StringPosition = lineStartingChar;
                    m_StringSelectPosition = lineStartingChar;
                    m_CaretPosition = lineStartingChar;
                    m_CaretSelectPosition = lineStartingChar;
                    Append("  ");
                    numLine++;
                }
                m_StringPosition = tempStringPosition + 2;
                m_StringSelectPosition = tempStringSelectPosition + 2 * numLine;
                m_CaretPosition = tempCaretPosition + 2;
                m_CaretSelectPosition = tempCaretSelectPosition + 2 * numLine;
            }
        }

        private void AppendShiftTab()
        {
            // In order to shift tab, we want to look at each line, and check if the first two characters of the
            // line are ' ' and if they are we should remove them, we'll manually edit m_Text
            int stringPositionLine = m_TextComponent.textInfo.characterInfo[m_StringPosition].lineNumber;
            int stringSelectPositionLine = m_TextComponent.textInfo.characterInfo[m_StringSelectPosition].lineNumber;
            int minLine = (int)Mathf.Min(stringPositionLine, stringSelectPositionLine);
            int maxLine = (int)Mathf.Max(stringPositionLine, stringSelectPositionLine);
            ...
            {
                int lineStartingChar = m_TextComponent.textInfo.lineInfo[i].firstCharacterIndex;
                if (m_Text[lineStartingChar] == ' ')
                {
                    if (m_Text[lineStartingChar + 1] == ' ')
                    {
                        m_Text = m_Text.Remove(lineStartingChar, 2);
                        numDeletions += 2;
                    }
                    else
                    {
                        m_Text = m_Text.Remove(lineStartingChar, 1);
                        numDeletions += 1;
                    }
                }
            }
            if (numDeletions > 0)
            {
                m_StringPosition -= numDeletions;
                m_StringSelectPosition -= numDeletions;
                m_CaretPosition -= numDeletions;
                m_CaretSelectPosition -= numDeletions;
                SendOnValueChanged();
            }
        }
        ...
        private void SendOnValueChanged()
        {
            ...
            undoRecorder.Record(new Record(
                m_Text,
                m_StringPosition,
